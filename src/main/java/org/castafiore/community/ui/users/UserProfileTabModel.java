...
public class UserProfileTabModel implements TabModel{
	...
	public UserProfileTabModel(User user, SecurityService service) {
		super();
	...
	public Container getTabContentAt(TabPanel pane, int index) {
		if(index == 0){
			return new EXAccountSettingForm(user, service);
		}else if(index == 1){
			if(user == null || user.getUsername()==null){
				throw new UIException("Please save the user first");
			}
			
			return new EXProfileForm(user, service);
		}else if(index == 2){
			if(user == null || user.getUsername()==null)
				//return new EXPermissionTab("", null);
				throw new UIException("Please save the user first");
			else
				return new EXPermissionTab("", user.getUsername(), service);
		}else if(index == 3){
