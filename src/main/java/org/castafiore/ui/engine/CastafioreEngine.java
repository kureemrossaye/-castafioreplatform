...
public final class CastafioreEngine {
	...
	// LogFactory.getLog(CastafioreEngine.class);
	...
	public synchronized String getJQuery(Container container, String parentId, Container root, ListOrderedMap buffer) {
		...
		if (!container.rendered()) {
			...
			// }else{
			//
			// container.setAttribute("__path", parent.getAttribute("__path") +
			// "/" + parent.getChildren().indexOf(container));
			// }
			...
			/**
			 * first we get previous and next sibbling to guess the best way to
			 * append the child
			 */
			...
			if (!(container.getParent() instanceof TemplateComponent)) {
				...
				// creationCommand = Constant.NO_CONFLICT +
				// "(\""+Constant.ID_PREF +root.getId()+"
				// *[__path^='"+container.getAttribute("__path")+"']\").remove();";
				// builder.append(Constant.NO_CONFLICT +
				// "(\"#"+container.getId()+"\").remove();");
			}
			...
			creationCommand = creationCommand
					+ JQuery.getCreationCommand(container, parentId, html, previousSibbling, nextSibbling);
		...
		String script = creationCommand + builder.toString();// getLazyScriptLoad(container,
																// builder,
																// creationCommand);
	 ...
	 * 
	 * @param componentId
	 *            - the component on which the event is registered
	 * @param eventType
	 *            - the type of event registered
	 * @param application
	 *            - the application in which the component belongs
	 * @param parentId
	 *            - the parentId of the component
	 * @param parameters
	 *            - the parameters passed as parameter
	 * @return returns the JQuery necessary to make the update
	 */
	public synchronized String executeServerAction(String componentId, Application application, String parentId,
		...
		if (component != null) {
			...
			// we don't do anything in particular in this case, since Castafiore
			// is for creating extremely rich application.
			// it is likely to have race condition in some extreme cases.
			// it is up to the user of the framework to make necessary
			// correction in program execution
			// logger.warn("the component " + componentId + " cannot be found in
			// dom. Possibly deleted");
		...
		if (event != null && event.hashCode() == eventId) {
			try {
				...
				// logger.debug("executing events, or rather your business
				// logic");
				...
				if (updateClient) {
					// logger.debug("action returned true, meaning client should
					// be updated");
					...
					// result.append(Constant.NO_CONFLICT +
					// "(\".castatohide\").css(\"display\", \"none\");\n");
				...
				/**
				 * @todo UIException should be handled by showing a div, with
				 *       the stacktrace in it
				 * @todo other exceptions should be thrown to j2ee container
				 *       since they are most probably technical exception
				 *       preferable do something scalable. Probably redirect to
				 *       some page, show a div or whatever. This will probably
				 *       depend on the developer a good idea would be to
				 *       delegate the handling of the exception on an interface
				 *       loaded via spring.
				 */
	...
	/**
	 * returns the previous sibbling , current and next sibbling in an array of
	 * 3 elements
	 * 
	 * @param container
	 * @return
	 ...
	 * 
	 * @param container
	 *            - the container that contains the event
	 * @param root
