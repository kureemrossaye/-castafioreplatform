...
public class EXTable extends EXContainer implements Table, Event {
	...
	public void refreshData() {
		...
		if (size == 0) {
			...
			tbody.addChild(tr);
			Container td = new EXContainer("emtpytd", "td").setAttribute("colspan", "1000").setText("No records found")
	...
	public void onReady(JQuery query) {
		try {
			...
			JQuery bootpag = query.clone().getDescendentById(paginator.getId()).invoke("bootpag", options).invoke("on",
					"page", function);
