...
public class SpringApplicationRegistry implements ApplicationRegistry, ApplicationContextAware {
	...
	public Application getApplication(HttpServletRequest request, HttpServletResponse response) {
		...
		if(applicationId == null){
			throw new RuntimeException("cannot load application. Please specify parameter casta_applicationid=<appname>");
		}
		try
		{
			
			Application application = (Application)context_.getBean(applicationId);
			return application;
		}
